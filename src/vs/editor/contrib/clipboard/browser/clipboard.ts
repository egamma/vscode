...
export function computeSupportsPaste(clipboardObj: Clipboard | undefined, queryCommandSupported: (command: string) => boolean): boolean {
	return (typeof clipboardObj === 'undefined') ? queryCommandSupported('paste') : true;
}
const supportsPaste = computeSupportsPaste(navigator.clipboard, document.queryCommandSupported.bind(document));
